...
export class GenocideEffect extends Component {
    private _radius = 30;
    ...
    private _startVFX(glowFrame: SpriteFrame | null): void {
        ...
        const outerNode = new Node('GnOuter');
        ...
        const innerNode = new Node('GnInner');
                ...
                if (!this._detaching && outerNode.isValid) {
                    this._pulseTween = tween(outerNode)
                        .repeatForever(tween<Node>()
                            .to(0.55, { scale: new Vec3(1.20, 1.20, 1) }, { easing: 'sineInOut' })
                            .to(0.55, { scale: new Vec3(1.0,  1.0,  1) }, { easing: 'sineInOut' }))
        ...
        this.schedule(this._spawnSparkle, 0.11);
    }
    ...
    private _fadeOut(): void {
        ...
        if (outerOp?.node?.isValid) tween(outerOp).to(0.5, { opacity: 0 }).start();
        if (innerOp?.node?.isValid) tween(innerOp).to(0.5, { opacity: 0 }).call(finish).start();
        else finish();
    }
